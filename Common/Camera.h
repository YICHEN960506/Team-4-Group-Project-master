...
namespace NCL {
	...
	public:
		Camera(void) {
			...
			this->offSet = Vector3(100, 75, 0);
			this->rotOffset = Vector3(150, 100, 150);
		...
		//
		void SetFollowTarget(bool follow, Vector3 target = Vector3(0, 0, 0)) {
			followTarget = follow; 
			if (follow) { targetPos = target; FollowTarget(); } }
	...
	protected:
		...
		//Following a target
		bool followTarget;
		Vector3 targetPos, offSet, rotOffset;
	};
}